...
$(document).on('click','.addmore',function(){
 ...
 $('#size_select').change(function(){
    ...
    if(selectValue == 'TermsCondition'){
      $('.iconHref').attr('href','/edit-termCondition')
        selOp = selectValue;
    ...
    $.ajax({ 
      ...
      success: function(response) {
        ...
        // $('.logs-side').html(response.sidedes);
         $('.termDesc,.faq-div,.privacyPol,.prodiv,.pro_area,.pre_are').addClass('d-none');
         $('.term-condition,.edit-icon,.multiSection').removeClass('d-none');
       }else if(response.status == "faq"){
            $('.faq-panel').html(response.result);
            $('.faq-div,.faq-panel,.multiSection').removeClass('d-none');
            $('.termDesc,.prodiv,.pro_area,.pre_are,.term-condition,.edit-icon,.privacyPol').addClass('d-none');
      
         }else if(response.status == "PrivacyPolicy"){
        
            $('.privacyPol').html(response.result);
            $('.privacyPol,.edit-icon,.multiSection').removeClass('d-none');
            $('.termDesc,.term-condition,.faq-div,.faq-panel,.prodiv,.pro_area,.pre_are').addClass('d-none');
         
         }else if(response.status == "Profanity"){
            $('.pro_area,.prodiv,.pre_are').removeClass('d-none');
            $('.profanity_div').html(response.result);
            $('.pre_are').html(response.areaHtml);
            $('.privacyPol,.term-condition,.termDesc,.faq-div,.faq-panel,.edit-icon,.multiSection').addClass('d-none');
