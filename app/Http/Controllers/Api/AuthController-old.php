...
class AuthController extends Controller
{
   ...
   public function linkedinauth(Request $request){
        ...
        $client = new \GuzzleHttp\Client(['base_uri' => 'https://api.linkedin.com/v2/emailAddress?q=members&projection=(elements*(handle~))']);
        $headers = [
            'Authorization' => 'Bearer ' . $request->token,
            'Accept'        => 'application/json',
        ];

        $response = $client->request('GET', 'https://api.linkedin.com/v2/emailAddress?q=members&projection=(elements*(handle~))', [
            'headers' => $headers
        ]);
        ...
        $myArray = json_decode(($responseData), true);
