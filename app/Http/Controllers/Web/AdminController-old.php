...
class AdminController extends Controller
{
    ...
    public function support(Request $request)
    {
        ...
        return view("admin.support")->with(['recruiterdata' => $recruiterdata, 'professionaldata' => $professionaldata, 'organizationdata' => $organizationdata, 'data' => $data]);
    }
    ...
    public function displayTicketMessage(Request $request){
        ...
        if ($supportdata) {
            ...
            $htmlContennt .=  '<p class="supp_chat_date"><span>' . $supportMessage->created_at.'</span></p>';
            $htmlContennt .=  '</div>';
            ...
            $htmlContennt .= '</div>';
            $htmlContennt .= '<p>'.$supportMessage->created_at.'</p>';
            $htmlContennt .= '</div>';
            $htmlContennt .= '</div>';
            ...
            foreach ($supportdata as $sdata) {
                ...
                $htmlContennt .=  '<div class="mt-4">';
                $htmlContennt .=  '<p class="supp_chat_date"><span>' . $created_at . '</span></p>';
                $htmlContennt .=  '</div>';
  ...
  if($role == "Professional" || $role == "Recruiter"){
    ...
    if(isset($data) && count($data) > 0){
        foreach($data as $d){
          ...
          if($role == "Professional"){
              $customer_img = URL('').'/images/'.$d->image;
          }else{
            if($d->userinfo){
   ...
   $i = 0;
    if(isset($data) && count($data) > 0){
        foreach($data as $d){
            ...
            $company_img =  URL('').'/images/'.$d->userinfo->profile_pic;
            $responseHtml .='<tr>';
    ...
    public function guidLine(Request $request){
            ...
            if($term == "TermsCondition" || $term == "PrivacyPolicy"){
                ...
                $sidedes = '';
                    ...
                    $sidedes .='</div>';
                    if($term == "TermsCondition"){
                        return response()->json(['result' => $ad,'sidedes'=>$sidedes,'status' => "TermsCondition"]);
                        }else{
